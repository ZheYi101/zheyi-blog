...
## 前置步骤-服务端渲染
这一步对于用户来说并不能算一个第一步, 因为往往服务器都是提前把**html**, **js文件**渲染好的, 用户(客户端)只要等着获取就行了;

需要注意, html和js文件这里是分开的, 正如我前面所说。 **js文件**会在后面通过`客户端水合` 嵌到html上
...
```
也是会的, 也就是即使是import操作, 也会在服务端渲染的时候执行
...
### 无法打请求
这个就同理了, 在服`前置步骤-服务端渲染`的时候是不能打请求的,
